...
function checkImportData(tableFields: string[], tableTitles: string[], fields: string[]) {
	return fields.some((field) => tableFields.indexOf(field) > -1 || tableTitles.indexOf(field) > -1);
}
		...
		if (readerTarget) {
			workbook.xlsx.load(readerTarget.result as ArrayBuffer).then((wb) => {
				...
				if (firstSheet) {
					...
					const status = checkImportData(tableFields, tableTitles, fields);
					if (status) {
