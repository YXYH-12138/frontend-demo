import { onMounted, shallowRef, onBeforeUnmount, createVNode } from "vue";
...
export const isPointMarker = (feature: FeatureLike) =>
  has(feature.getProperties(), IS_MARKER);
...
export class OlMapVue extends OlMap {
  ...
  calculateCenter(extent: number[], padding?: number | number[]) {
    ...
    resolution = view.getConstrainedResolution(
      resolution,
      nearest ? 0 : 1,
    ) as number;
  ...
  onMoveMarker(
    callback: (features: FeatureLike[], e: MapBrowserEvent<any>) => void,
  ) {
