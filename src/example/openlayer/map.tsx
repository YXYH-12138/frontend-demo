...
import { TIAN_TK, TIAN_VEW_W } from "@/constants";
...
function createWMTSSource(layerName: string, formatType = "tiles"): WMTS {
  ...
  const tileGrid = new WMTSTileGrid({
    origin: getTopLeft(projectionExtent) as [number, number],
    resolutions: resolutions,
    matrixIds: matrixIds,
  });

  const [layer] = layerName.split("_");

  return new WMTS({
    url: `https://t{0-7}.tianditu.gov.cn/${layerName}/wmts?tk=${TIAN_TK}`,
    layer,
    matrixSet: "w",
    format: formatType,
    projection,
...
export function useMarker(map: HomeMap) {
  ...
  const { dispose, initPopup, changeTooltipVisible, initPopupTitle } =
    useTooltip(map, markerFeatures);
